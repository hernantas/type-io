...
describe('[BSON] Bson Parser (Double Codec)', () => {
  ...
  it('Decode/Encode from Double', () => {
    ...
    const decode = parser.decode(value, variant(Number, Double))
    const encode = parser.encode(decode, variant(Number, Double)) as Double
  ...
  it('Decode/Encode from Boolean', () => {
    const tDec = parser.decode(true, variant(Number, Double))
    const tEnc = parser.encode(tDec, variant(Number, Double)) as Double
    expect(tDec).to.equal(1)
    expect(tEnc.value).to.equal(1)

    const fDec = parser.decode(false, variant(Number, Double))
    const fEnc = parser.encode(fDec, variant(Number, Double)) as Double
