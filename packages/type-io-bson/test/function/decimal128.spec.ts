...
describe('[BSON] Bson Parser (Decimal128 Codec)', () => {
  ...
  it('Decode/Encode from Decimal128', () => {
    ...
    const decode = parser.decode(value, variant(String, Decimal128))
    const encode = parser.encode(decode, variant(String, Decimal128))
  ...
  it('Decode/Encode from Boolean', () => {
    const tDec = parser.decode(true, variant(String, Decimal128))
    const tEnc = parser.encode(tDec, variant(String, Decimal128))
    expect(tDec).to.equal('1')
    expect(String(tEnc)).to.equal('1')

    const fDec = parser.decode(false, variant(String, Decimal128))
    const fEnc = parser.encode(fDec, variant(String, Decimal128))
